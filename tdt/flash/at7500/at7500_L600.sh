...
if [ -e $TUFSBOXDIR/release ] && [ -e $TUFSBOXDIR/release_neutrino ]; then
  echo "Found targets:"
  echo "   1) Enigma2"
  echo "   2) Neutrino"
  ...
  case "$REPLY" in
    ...
    2) echo "Preparing Neutrino Root..."
      $SCRIPTDIR/prepare_root_neutrino.sh $CURDIR $TUFSBOXDIR/release_neutrino $TMPROOTDIR $TMPKERNELDIR $OWNLANG $OWNCOUNTRY;;
    *) echo "Invalid Input! Exiting..."
      exit 2;;
  esac
elif [ -e $TUFSBOXDIR/release ] && [ ! -e $TUFSBOXDIR/release_neutrino ]; then
  echo "Preparing Enigma2 Root..."
  $SCRIPTDIR/prepare_root.sh $CURDIR $TUFSBOXDIR/release $TMPROOTDIR $TMPKERNELDIR $OWNLANG $OWNCOUNTRY
elif [ ! -e $TUFSBOXDIR/release ] && [ -e $TUFSBOXDIR/release_neutrino ]; then
  echo "Preparing Neutrino Root..."
  $SCRIPTDIR/prepare_root_neutrino.sh $CURDIR $TUFSBOXDIR/release_neutrino $TMPROOTDIR $TMPKERNELDIR $OWNLANG $OWNCOUNTRY
else
  echo "Nothing to do! Have you built an image?"
  exit
fi
echo "Root prepared."
echo ""
